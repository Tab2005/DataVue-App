...
def _openrouter_classifier():
    with patch("services.ai.intent_classifier.OpenRouterClient") as mock_or_cls:
        classifier = AIIntentClassifier(api_key="fake-key")
    return classifier, mock_or_cls.return_value


def test_classify_queries_uses_streaming_call_for_openrouter():
    """
    Regression test: search intent classification must use generate_content_stream
    (same call pattern as the working GA4 insights AI analysis feature and
    AIContentGapSuggester), not the blocking generate_content, since some
    free/rate-limited upstream models silently return empty content on the
    non-streaming path.
    """
