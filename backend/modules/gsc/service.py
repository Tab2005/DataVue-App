...
真實實作（docs/07_audits_and_reviews/CODE_REVIEW_ACTION_PLAN_2026-07-01.md
P1-1 真遷移，比照 docs/22 第 0 波 GA4 的遷移方式）。根目錄 `gsc_service.py`
保留為薄轉發層，供既有呼叫端（`modules/gsc/router.py`、
`scripts/verify_gsc_search_appearance.py`）的
`from gsc_service import GSCService` 匯入路徑與方法簽名不變；新代碼請
直接 import `modules.gsc.service`，不要在 `gsc_service.py` 新增邏輯。
"""
...
__all__ = ["GSCService", "get_analytics_data"]
