...
_FALLBACK_USER_PROMPT_TEMPLATE = (
    ...
    "2. Text Ratio & Legibility: Are the advertiser's OWN overlay copy elements (headline, subhead, banner text, "
    "CTA badges added on top of the photo) readable, and is their text-to-image ratio balanced (avoiding overloaded "
    "ad copy)? Do NOT evaluate or penalize the legibility of text printed on the product's own packaging/label "
    "(ingredient lists, regulatory warnings, usage instructions, barcodes, etc.) — that text is fixed by the "
    "product manufacturer and is outside the advertiser's control, so it must be excluded from this criterion.\n"
    ...
    "  - copywriting: Evaluates headline and primary text persuasiveness, and legibility of the advertiser's OWN "
    "overlay text only — explicitly EXCLUDE text printed on the product's own packaging/label from this evaluation.\n"
