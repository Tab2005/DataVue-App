"""
AI Hub Module - Service

真實實作（docs/07_audits_and_reviews/CODE_REVIEW_ACTION_PLAN_2026-07-01.md
P1-1 真遷移，比照 docs/22 第 0 波 GA4 的遷移方式）。根目錄 `ai_service.py`
保留為薄轉發層，供既有呼叫端（`modules/ai_hub/router.py`、
`tests/test_analytics_ai.py`、`tests/test_ga4_insights_wave2.py`）的
`from ai_service import AIService` 匯入路徑與方法簽名不變；新代碼請
直接 import `modules.ai_hub.service`，不要在 `ai_service.py` 新增邏輯。
"""
