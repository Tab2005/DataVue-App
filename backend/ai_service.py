...
class AIService:
    ...
    def analyze_data(
        ...
        if report_type == "contribution_analysis":
            ...
            7. **語言**：繁體中文（台灣市場用語，例如「工作」「曝光」），Markdown 格式，關鍵數字加粗。

            報告結構（依此順序產出四段）：
            1. **一句話總結**：這次分析最重要的一件事，給不懂統計的主管第一個 takeaway。
            2. **功勞被高估 / 被低估的組別**：對照自報占比（平台後台報的）vs MMM 貢獻，點名哪些組是「收割型」（自報高、MMM 低）、哪些是「被低估的真實引擎」（MMM 高、自報低）。
            3. **下一塊錢建議投給誰、為什麼**：以邊際報酬排序為依據，點出最高邊際組別並簡述理由；但若最高邊際組別被標為存疑，必須註明「但因共線性，結論待釐清」。
            4. **這份分析要保留的地方**：把 diagnostics 內的共線性警告、Poisson 天花板、holdout R² 翻成白話，說明「哪些結論是穩的、哪些還需要更多資料才能下定論」。
            """
