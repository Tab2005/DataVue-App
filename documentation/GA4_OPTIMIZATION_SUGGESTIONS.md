...
## 已實作的 GA4 佈署端優化（2026-01-20）

以下功能已落地，並與 GSC 同步採用相同優化模式：

1. **後端分頁 + `limit/offset`**
    - GA4 `/api/ga4/report` 支援分頁查詢
    - 預設只取一頁，前端再逐頁載入

2. **Redis 快取（佈署端）**
    - 有 `REDIS_URL` 時使用 Redis
    - 新增 `GA4_REDIS_TTL_SECONDS` 控制快取 TTL
