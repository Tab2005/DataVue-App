...
| 層級 | 說明 | 儲存方式 | 是否隨日期變化 |
|------|------|----------|----------------|
| **關鍵字意圖** | 每個關鍵字的固定意圖 | LocalStorage / 資料庫 | ❌ 不變 |
| **文章意圖** | 動態計算，不儲存 | 即時計算 | ✅ 會變 |

**快取結構** (關鍵字級):
```javascript
// LocalStorage: 'gsc_keyword_intents'
{
  "老鼠怕什麼": { "intent": "informational", "confidence": 0.92 },
  "老鼠藥推薦": { "intent": "commercial", "confidence": 0.88 }
}
```
...
| 元素 | 行為 |
|------|------|
| 展開的關鍵字列表 | 隨日期區間變化 (GSC 即時資料) |
| 關鍵字意圖標籤 | 查詢快取，固定值 |
| 文章意圖標籤 | 動態計算 (基於當前日期 Top 10) |
