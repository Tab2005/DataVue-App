...
本報告列出的 P0/P1/P2 項目已全數實作完成並合併至 main；P3 中 A8、B7 依文件本身建議維持不做；B2 兩層（前端併發化＋後端批次端點）皆已完成。逐項對照：
...
| B1 前端匯入狀態輪詢 | ✅ 已完成 | |
| B2 批次匯入效率 | ✅ 已完成 | 第一層：前端改有限併發（`BATCH_IMPORT_CONCURRENCY`）。第二層：新增 `POST /evaluations/import/facebook-ads/batch` 後端批次端點，派工前先預熱一次整包報告快取（`prewarm_facebook_ads_report_cache`），逐筆仍沿用既有 `queue_observed_facebook_ad_import()`（含 B5 冪等去重）與派工路徑；前端尚未改接此批次端點，仍是逐筆呼叫單筆端點 |
| B3 影片廣告匯入支援 | ✅ 已完成 | |
| B4 媒體下載改串流限制大小 | ✅ 已完成 | |
| B5 匯入端點冪等去重 | ✅ 已完成 | |
