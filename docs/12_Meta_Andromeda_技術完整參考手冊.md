# 12 Meta Andromeda 技術完整參考手冊

> 本文件依據 2026-06-30 當前程式碼庫實際狀態撰寫，涵蓋 Meta Andromeda 模組的資料庫模型、API 端點、評分引擎、佇列主機、儲存後端、前端頁面與維護機制，供開發、部署、稽核人員作為首要技術參考。
...
### 3.4 MetaAndromedaFeedbackEvent（人工回饋事件，已退出主流程）

**資料表名**：`meta_andromeda_feedback_events`

> **注意**：「評估紀錄」頁面改版後，此表已不再被前端主動寫入。表結構與已有資料保留，API endpoint（`/scores/{id}/feedback`）仍存在可用，但前端 UI 已移除審核表單。
...
### 3.12 MetaAndromedaScoringProfile（AI 評分設定檔）

**資料表名**：`meta_andromeda_scoring_profiles`

儲存 AI 評分引擎的 System Prompt、User Prompt Template 及各廣告目標群組的專屬覆蓋設定。由 Alembic 遷移 `20260624_add_meta_andromeda_scoring_profiles` 建立，`objective_profiles` 欄位於 `20260630_meta_andromeda_objective_profiles` 新增。

| 欄位 | 類型 | 說明 |
|---|---|---|
| `id` | Integer PK | 自增主鍵 |
| `name` | String UNIQUE | 設定檔名稱（如 `default`） |
| `is_active` | Boolean | 是否為當前生效設定檔 |
| `system_prompt` | Text | 全域 System Prompt |
| `user_prompt_template` | Text | 預設 User Prompt 模板（支援 `{headline}`、`{primary_text}`、`{cta}` 佔位符） |
| `few_shot_examples` | JSON | Few-shot 範例列表（務必以 Python list 寫入，見 §5.6 陷阱 A） |
| `objective_profiles` | JSON | 各廣告目標群組的專屬覆蓋設定（見 §5.7） |
| `created_at` / `updated_at` | DateTime | 建立與更新時間 |

---

### 3.13 MetaAndromedaLabelPolicy（觀測標籤動態門檻）

**資料表名**：`meta_andromeda_label_policies`

持久化每個 `(scope_key, window_kind)` 組合（`scope_key` 為廣告帳號 ID，或 `"global"`）算出的動態 P33/P67 標籤門檻，供 `create_drift_report`（漂移報告）與 `sync_calibration_dataset`（校準集同步）共用，確保同一批 `ObservedCreative` 在兩條 pipeline 標出一致的 `observed_band`；也是單筆查詢（`get_review_queue_detail`）在樣本不足時的 fallback 來源。計算與寫入邏輯在 `backend/modules/meta_andromeda/labeling.py`（`compute_label_thresholds()` / `persist_label_policy()`）。由 Alembic 遷移建立，`cpm_*` 四欄由 `20260714_ma_label_policy_cpm` 新增。

| 欄位 | 類型 | 說明 |
|---|---|---|
| `id` | String PK | `ma_lp_{uuid}` |
| `scope_key` | String(120) | 廣告帳號 ID 或 `"global"` |
| `window_kind` | String(50) | 觀測時間窗種類（如 `last_30d`） |
| `label_policy_version` | String(50) | 標籤策略版本號 |
| `roas_low` / `roas_high` / `roas_method` / `roas_sample_count` | Float / Float / String(30) / Integer | conversion/app/unknown 組的 ROAS 門檻（越高越好） |
| `ctr_low` / `ctr_high` / `ctr_method` / `ctr_sample_count` | Float / Float / String(30) / Integer | TRAFFIC/ENGAGEMENT/VIDEO 組（`CTR_CPC_GROUPS`）的 CTR 門檻（越高越好） |
| `cpc_low` / `cpc_high` / `cpc_method` / `cpc_sample_count` | Float / Float / String(30) / Integer | `CTR_CPC_GROUPS` 組在無 CTR 資料時退用的 CPC 門檻（越低越好） |
| `cpm_low` / `cpm_high` / `cpm_method` / `cpm_sample_count` | Float / Float / String(30) / Integer | **AWARENESS 組專用**（2026-07-14 新增）：CPM（每千次曝光成本）門檻，越低越好；AWARENESS 觸及/曝光效率廣告不適用 CTR/CPC |
| `cvr_low` / `cvr_high` / `cvr_method` / `cvr_sample_count` | Float / Float / String(30) / Integer | LEAD 組的 CVR 門檻（越高越好） |
| `cpl_low` / `cpl_high` / `cpl_method` / `cpl_sample_count` | Float / Float / String(30) / Integer | LEAD 組在無 CVR 資料時退用的 CPL 門檻（越低越好） |
| `effective_from` / `created_at` | DateTime | 生效時間與建立時間 |

`method` 欄位可能的值：`percentile_p33_p67`（本批樣本 ≥ 20 筆，動態百分位）、`persisted_prior`（樣本不足，沿用上一期已持久化門檻）、`fixed_fallback`（無足夠樣本也無歷史門檻，回退至 `labeling.py` 中的固定常數，如 `_CTR_FALLBACK_LOW/_HIGH`、`_CPM_FALLBACK_LOW/_HIGH`）。
...
| Method | 路徑 | 說明 |
|---|---|---|
| GET | `/review-queue` | 列出評分事件（支援 `status`、`has_observation`、`limit` 篩選） |
| GET | `/review-queue/{score_event_id}` | 取得單一評分事件詳情（含 `observation` 實際成效對照） |

**`has_observation` 篩選說明**：`true` = 已與 `CalibrationItem` 匹配（有實際成效數據）；`false` = 尚未匹配；省略 = 全部。

**詳情回應 `observation` 欄位**（有匹配時）：
```json
{
  "observation": {
    "prediction_band": "mid",
    "observed_band": "low",
    "error": 1.0,
    "performance_snapshot": { "spend": 1200, "roas": 1.2, ... },
    "ad_name": "廣告名稱",
    "observation_window_kind": "last_7d",
    "observation_window_start": "2026-06-01",
    "observation_window_end": "2026-06-07"
  }
}
```
...
#### 目標群組分類（`_OBJECTIVE_GROUP_MAP`）

所有廣告目標字串（來自前端輸入或 Facebook API）統一對應到 6 個群組：

| 群組 | 代表目標值（節錄） | 適用評估指標 | `roas_band` 有效 |
|---|---|---|---|
| `conversion` | `purchase`、`OUTCOME_SALES`、`conversions` | ROAS | ✓ |
| `lead` | `lead`、`OUTCOME_LEADS`、`lead_generation` | CVR / CPL | ✓ |
| `traffic` | `link_clicks`、`OUTCOME_TRAFFIC` | CTR / CPC | ✗ |
| `awareness` | `brand_awareness`、`OUTCOME_AWARENESS`、`reach` | CPM（每千次曝光成本） | ✗ |
| `video` | `video_views`、`OUTCOME_VIDEO_VIEWS` | VTR / Hook 率（觀測端目前仍用 CTR / CPC） | ✗ |
| `engagement` | `post_engagement`、`OUTCOME_ENGAGEMENT` | 互動率（觀測端目前仍用 CTR / CPC） | ✗ |

不在對照表內的目標值預設對應 `conversion` 群組。

「適用評估指標」欄是模型 prompt 的評分側重方向；**觀測端（實際成效 `observed_band`）的指標路由**另見 `objective_routing.py` 的 `CTR_CPC_GROUPS`（traffic/engagement/video）與 `labeling.py` 的 AWARENESS 專屬 CPM 分支（2026-07-14 起，見 §3.13 與 `docs/16` 文末修正說明）——`awareness` 是唯一一組「prompt 側重」與「觀測端指標」完全一致（都是 CPM／曝光效率）的目標；`video`/`engagement` 的觀測端目前仍是 CTR/CPC 代理指標，尚未拆分成 VTR/互動率專屬公式。

#### 設定檔三層優先級（`_resolve_active_profile()`）

```
DB: objective_profiles[group]           ← 最高優先（可線上調整）
  └─ _DEFAULT_OBJECTIVE_PROFILES[group] ← 硬編碼預設（嵌入 runtime.py）
       └─ base_profile                  ← 全局設定檔（conversion 行為）
```

- `_DEFAULT_OBJECTIVE_PROFILES` 硬編碼於 `runtime.py`，保證即使 DB 遷移失敗仍能正確路由
- `roas_band_eligible` 為 `False` 時，評分結果的 `roas_band` 欄位設為 `null`

#### 各目標群組的 Diagnostic Keys

| 群組 | 診斷細項 |
|---|---|
| `conversion` | `visual_appeal`、`copywriting`、`cta_clarity`、`relevance` |
| `lead` | `visual_appeal`、`trust_signals`、`value_proposition`、`audience_fit`、`cta_clarity` |
| `traffic` | `thumb_stop`、`curiosity_hook`、`landing_relevance`、`cta_clarity` |
| `awareness` | `brand_recall`、`message_clarity`、`visual_distinctiveness`、`emotional_resonance` |
| `video` | `hook_strength`、`pacing`、`message_delivery`、`brand_integration` |
| `engagement` | `shareability`、`emotional_hook`、`interaction_trigger`、`visual_impact` |

繁體中文顯示標籤定義於 `frontend/src/utils/metaAndromedaLabels.js` 的 `DIAGNOSTIC_KEY_LABELS_ZH`。

#### 已知陷阱 D：DB 遷移 UPDATE JSON 欄位失敗靜默

**症狀**：部署後 non-conversion 廣告（如知名度廣告）仍使用 ROAS 評分 Prompt。

**原因**：Alembic migration 以 `UPDATE ... SET objective_profiles = :val` 寫入 JSON 欄位時，psycopg2 的 JSON coercion 可能靜默失敗，欄位維持 `NULL`。`_resolve_active_profile()` 拿到空 dict，一路 fallback 至全局設定檔（conversion 行為）。

**修正**：`_DEFAULT_OBJECTIVE_PROFILES` 硬編碼常數是生產環境的實際安全網，DB override 是可選的線上微調機制。
...
| `metaAndromedaReviewQueueService.js` | `/review-queue`、`/review-queue/{id}` |
