...
- 建立日期:2026-07-13
- 狀態:第 1、2、3、4 波已完成;第 5 波待第 1 波歸因結論後啟動;2026-07-14 額外修復一個不屬於任何一波規劃的觀測端標籤 bug(見文末「八」),**啟動第 5 波前建議先讀**
...
## 八、2026-07-14 追加:觀測端標籤 bug 修復(非本計劃五波之一,意外發現)

用戶核對「評估紀錄」明細頁時,發現 traffic/awareness 廣告的「實際成效」不論真實表現一律顯示 LOW,追查後定位並修復兩個問題(不屬於上述五波中任何一波——是觀測端 `labeling.py` 標籤計算本身的 bug 與指標選型錯誤,跟評分模型準不準無關,即使模型判斷完全正確,只要觀測端標籤算錯,準確率分析都會失真):

1. **`get_review_queue_detail()`(`repository.py`)呼叫 `label_observed_band()` 漏傳 `label_thresholds`**:只影響「成效分析匯入」單筆查詢路徑(前端評估紀錄明細頁即時顯示),**不影響**本計劃引用的 docs/30 §2 331 筆 pairwise accuracy 批次計算(該批次路徑本來就有正確傳門檻)。
2. **AWARENESS 誤用 CTR/CPC、應改用 CPM**:AWARENESS(觸及/曝光型)廣告的優化目標不是點擊,CTR 高低跟這類廣告表現沒有必然關係。這個問題**會**影響 docs/30 §2 的 331 筆批次計算與 §4.1 第 1 項要做的人工抽樣歸因,因為 docs/23 完成後 traffic/awareness/video/engagement 四組都會進入 pairwise accuracy 計算,AWARENESS 廣告的 `real_band` 在修復前是用不相關的 CTR 算出來的。

詳見 `docs/16` 文末「2026-07-14 後續修正」、`docs/23` §7、`docs/30` §7(含影響評估與後續建議)。

**對本計劃第 1 波與第 5 波的影響**:
- 若第 1 波任務 1.1/1.2 的人工抽樣歸因(高分低效/低分高效各 15 筆)是在 2026-07-14 修復**之前**做的,樣本可能混有因指標誤用而錯標的 AWARENESS 案例,歸因結論(假設 1 模型能力不足 vs 假設 2 rubric 錯位)可能需要重新檢視,尤其若當初抽樣結果中 AWARENESS 素材占比不低。
- 啟動第 5 波(rubric / 級距機制重設計)前,建議先重新執行一次 `refresh-metrics`,比較修復前後的 `pairwise_ranking_accuracy` / `mean_band_error` 有無變化,確認這次修復對 docs/30 §2 數字的實際影響幅度,避免在被污染的基準線上做路線判斷。
