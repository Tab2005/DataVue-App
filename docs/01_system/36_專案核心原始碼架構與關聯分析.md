...
> **⚠️ 2026-08-11 提醒**：本文件的行數統計（2026-07-17 產出）與部分核心檔案路徑已過時 —— 例如 `backend/routers/gsc.py` 已搬遷為 `backend/modules/gsc/router.py`，`Analytics.jsx`／`GA4Insights.jsx` 行數已進一步縮減。**現況架構與行數請改看 [71_專案現況架構與程式碼行數統計.md](71_專案現況架構與程式碼行數統計.md)**；本文件的模組關聯圖與資料鏈描述仍準確，予以保留。
