...
const OVERVIEW_LABELS = {
    zh: {
        ...
        statusLabels: {
            ...
            db_backed_promotable: '資料庫存儲、可升級',
            phase_3_prompt_calibration: '第三階段：Prompt 自適應校準',
            db_backed_scoring_profiles_and_calibration_pipeline: '資料庫評分 Profile 與校準流水線已就緒',
        ...
        notesMap: {
            ...
            'Scoring runtime resolves provider and model metadata from the local Meta Andromeda registry.': '評分執行程序會從本地 Meta Andromeda registry 解析 provider 與模型中繼資料。',
            'Queue host dispatch, worker audit, and dead-letter observability are persisted in DataVue DB.': '佇列主機派送、工作程序稽核與死信觀測資料已寫入 DataVue 資料庫。',
            'Scoring profiles are stored in DB and dynamically loaded at runtime; the promoted profile is used globally.': '評分 Profile 儲存於資料庫並在執行期動態載入，升級後的 Profile 全域生效。',
            'Calibration pipeline auto-generates corrected profiles when ≥10 mismatch items are synced.': '校準流水線在同步 ≥10 筆誤判資料後，自動產生修正版 Profile。',
